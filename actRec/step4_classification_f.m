function []=step4_classification_f()
% 分类，得到最后结果
% step1:kmean聚类
% step2:svm分类，直方图交叉核
...
for i = 1:tr_num
    ...
    for j = 1:tr_num            
        ...
        ktrain2(i,j) = sum(mintmp);    

    end
end
Ktrain2 = [(1:tr_num)',ktrain2];
model_precomputed2 = libsvmtrain(train_label, Ktrain2, '-t 4');

disp('*****************************************************')

ktest2 = ones(tt_num,tr_num);
for i = 1:tt_num
    %disp(i)
    for j = 1:tr_num
        mintmp=zeros(1,m);
        ...
        ktest2(i,j) = sum(mintmp);
    end
end
Ktest2 = [(1:tt_num)', ktest2];
[predict_label_P2, accuracy_P2, dec_values_P2] = libsvmpredict(test_label, Ktest2, model_precomputed2);


% Display the accuracy
accuracyL = accuracy_L(1) % Display the accuracy using linear kernel
accuracyP2 = accuracy_P2(1) % Display the accuracy using precomputed kernel


% 将数据输出到txt文件保存
resultPath = sprintf('data\\result.txt');
disp(resultPath);
fid=fopen(resultPath,'wt');
fprintf('\n%s:',resultPath);
fprintf(fid,'linear:%f\ncross:%f\n',accuracyL,accuracyP2);
fclose(fid);
